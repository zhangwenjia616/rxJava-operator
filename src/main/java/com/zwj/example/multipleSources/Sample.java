...
public class Sample {
  ...
  public static void main(String[] args) {
    ...
    Observable<Data> source = Observable.concat(
        sources.memory(), // 先从内存中取 看是否是符合first中的条件的
        sources.disk(), // 再从磁盘里取
        sources.network()) // 再从网络上获取
        .first(data -> data != null && data.isUpToDate());

    // "Request" latest data once a second
    Observable.interval(1, TimeUnit.SECONDS).flatMap(__ -> source) // 跟输入的秒数无关 所以是__ 返回的是source
                                                                   // Observable 主要是返回source
        .subscribe(data -> System.out.println("##Received## " + data.value)); // 在这里订阅
